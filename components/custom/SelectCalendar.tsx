...
export function SelectCalendar({
	...
	return (
		<>
			...
			<DateRange date={new Date()} showDateDisplay={false} rangeColors={['#2462EA']} ranges={state} onChange={(item) => setState([item.selection])} minDate={new Date()} direction="vertical" disabledDates={disabledDates} />
		</>
	);
}
